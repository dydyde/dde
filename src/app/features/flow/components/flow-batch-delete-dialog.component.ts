        ...
        <div class="bg-white rounded-2xl shadow-2xl border border-stone-200 overflow-hidden animate-scale-in w-full"
          ...
          <div class="px-5 pt-5 pb-4">
            ...
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="text-lg font-bold text-stone-800">批量删除任务</h3>
                <p class="text-xs text-stone-500">此操作将移动到回收站</p>
              </div>
            </div>
            ...
            <div class="bg-stone-50 rounded-xl p-4 space-y-2">
              <div class="flex justify-between items-center">
                <span class="text-sm text-stone-600">选中任务</span>
                <span class="text-sm font-semibold text-stone-800">{{ d.impact.explicit }} 个</span>
              </div>
              @if (d.impact.cascaded > 0) {
                <div class="flex justify-between items-center">
                  <span class="text-sm text-amber-600">级联子任务</span>
                  <span class="text-sm font-semibold text-amber-700">+ {{ d.impact.cascaded }} 个</span>
                </div>
              }
              <div class="border-t border-stone-200 pt-2 flex justify-between items-center">
                <span class="text-sm font-medium text-stone-700">总计删除</span>
                <span class="text-base font-bold text-red-600">{{ d.impact.total }} 个任务</span>
              </div>
            </div>
          ...
          <div class="flex border-t border-stone-100">
            <button 
              (click)="cancel.emit()"
              class="flex-1 px-4 py-3.5 text-sm font-medium text-stone-600 hover:bg-stone-50 transition-colors">
              取消
            </button>
            <button 
              (click)="confirm.emit()"
