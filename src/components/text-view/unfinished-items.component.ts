          ...
          @for (item of store.unfinishedItems(); track trackUnfinished(item)) {
            <div class="p-2 bg-panel/50 backdrop-blur-sm rounded-lg border border-retro-muted/20 hover:border-retro-rust hover:shadow-sm cursor-pointer group flex items-start gap-2 active:scale-[0.98] transition-all">
              <button 
                ...
                title="点击完成"></button>
              <div class="flex-1 min-w-0" (click)="onJump(item.taskId)">
...
export class UnfinishedItemsComponent {
  ...
  /** 完成事件 */
  readonly complete = output<{ taskId: string; text: string }>();
  
  /** 跳转到任务事件 */
  readonly jumpToTask = output<string>();
  
  trackUnfinished = (item: UnfinishedItem) => `${item.taskId}-${item.text}`;
  
  onComplete(item: UnfinishedItem, event: Event) {
    event.stopPropagation();
    this.complete.emit({ taskId: item.taskId, text: item.text });
  }
  
  onJump(taskId: string) {
    this.jumpToTask.emit(taskId);
  }
}
