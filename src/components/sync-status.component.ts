...
export class SyncStatusComponent {
  ...
  // 计算属性 - 是否有需要关注的问题（包括 offlineMode 连接中断状态）
  readonly hasIssues = computed(() => 
    this.deadLetterCount() > 0 || this.pendingCount() > 0 || !!this.syncError() || this.offlineMode()
  );
  
  readonly totalIssues = computed(() => 
    this.deadLetterCount() + (this.pendingCount() > 0 ? 1 : 0) + (this.syncError() ? 1 : 0) + (this.offlineMode() ? 1 : 0)
  );
